...
void fixAlignmentAmbiguity(TypeAlignment *al) {
	TypeNumber n;
	TypePosition position;

	for(n=0; n<al->number; n++)
		for(position=0; position<al->size; position++)
			if(al->sequence[n][position]>=al->cardinal)
				al->sequence[n][position] = al->empty;
}
...
TypeSetOfSequences *toSequences(TypeAlignment *al) {
	TypePosition position;
	TypeNumber n, m;
	...
	res->name = al->name;
	res->table = al->table;
	res->cardinal = al->cardinal;
	res->ambiguity = al->ambiguity;
	res->size = (TypePosition*) monmalloc(al->number*sizeof(TypePosition));;
	res->sequence = (TypeSymbol**) monmalloc(al->number*sizeof(TypeSymbol*));
	res->totSize = 0;
	res->number = 0;
	for(n=0; n<al->number; n++) {
		TypePosition ind;
		res->sequence[res->number] = (TypeSymbol*) monmalloc((al->size+1)*sizeof(TypeSymbol));
		res->size[res->number] = 0;
		for(position=0; position<al->size; position ++)
			if(al->sequence[n][position] != al->empty)
				res->sequence[res->number][res->size[res->number]++] = al->sequence[n][position];
		if(res->size[res->number]>0) {
			res->sequence[res->number][res->size[res->number]] = -res->number-1;
			res->sequence[res->number] = (TypeSymbol*) monrealloc((void*) res->sequence[res->number], (res->size[res->number]+1)*sizeof(TypeSymbol));
			res->totSize += res->size[res->number];
			res->number++;
		} else {
			monfree((void*)res->sequence[res->number]);
		}
	}
	return res;
}
