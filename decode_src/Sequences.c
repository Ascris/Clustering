...
int comparePosition(void const *a, void const *b) {
   TypePosition const *pa = a;
   TypePosition const *pb = b;
   return *pa - *pb;
}
...
TypeSetOfSequences readSequencesFasta(FILE *f, char *table, int canInc) {
	...
	while(c != EOF) {
		...
		bufferSymbol[position] = -seq.number-1;
		if(position>=SIZE_MIN) {
			...
			seq.sequence[seq.number] = (TypeSymbol*) monrealloc(bufferSymbol, (position+1)*sizeof(TypeSymbol));
...
void fixSequencesAmbiguity(TypeSetOfSequences *set) {
	TypeNumber n;
	for(n=0; n<set->number; n++) {
		TypePosition position, i = 0;
		for(position=0; position<set->size[n]; position++)
			if(set->sequence[n][position]<set->cardinal)
				set->sequence[n][i++] = set->sequence[n][position];
		set->sequence[n][i] = -n-1;
		set->size[n] = i;
		set->sequence[n] = (TypePosition *) monrealloc((void*) set->sequence[n], (i+1)*sizeof(TypePosition));
	}
}
